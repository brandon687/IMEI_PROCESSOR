    ...
    def _update_order_status_sqlite(self, order_id: str, status: str, result_code: str = None,
        ...
        try:
            ...
            if result_data:
                ...
                # Parser-extracted fields (8 additional)
                if result_data.get('imei2'):
                    update_fields.append('imei2 = ?')
                    params.append(result_data['imei2'])
                if result_data.get('serial_number'):
                    update_fields.append('serial_number = ?')
                    params.append(result_data['serial_number'])
                if result_data.get('meid'):
                    update_fields.append('meid = ?')
                    params.append(result_data['meid'])
                if result_data.get('gsma_status'):
                    update_fields.append('gsma_status = ?')
                    params.append(result_data['gsma_status'])
                if result_data.get('purchase_date'):
                    update_fields.append('purchase_date = ?')
                    params.append(result_data['purchase_date'])
                if result_data.get('applecare'):
                    update_fields.append('applecare = ?')
                    params.append(result_data['applecare'])
                if result_data.get('tether_policy'):
                    update_fields.append('tether_policy = ?')
                    params.append(result_data['tether_policy'])
    ...
    def _update_order_status_supabase(self, order_id: str, status: str, result_code: str = None,
        ...
        try:
            ...
            if result_data:
                ...
                # Parser-extracted fields (8 additional)
                if result_data.get('imei2'):
                    update_data['imei2'] = result_data['imei2']
                if result_data.get('serial_number'):
                    update_data['serial_number'] = result_data['serial_number']
                if result_data.get('meid'):
                    update_data['meid'] = result_data['meid']
                if result_data.get('gsma_status'):
                    update_data['gsma_status'] = result_data['gsma_status']
                if result_data.get('purchase_date'):
                    update_data['purchase_date'] = result_data['purchase_date']
                if result_data.get('applecare'):
                    update_data['applecare'] = result_data['applecare']
                if result_data.get('tether_policy'):
                    update_data['tether_policy'] = result_data['tether_policy']
